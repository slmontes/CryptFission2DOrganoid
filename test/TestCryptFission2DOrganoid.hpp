/*
 * TestCryptFission2DOrganoid.hpp
 *
 * Created on: Mar 14 2015 
 * Original code by: Axel A. Almet 
 * Last modified: Feb 04 2020
 * 		Author: Sandra Montes
 */


#ifndef TESTCRYPTFISSION2DORGANOID_HPP_
#define TESTCRYPTFISSION2DORGANOID_HPP_
/*
 ...
 * Almet et al (2018) "A Multicellular Model of Intestinal Crypt Buckling and Fission"
 *
 ...
 */
class TestCryptFission2DOrganoid : public AbstractCellBasedTestSuite
{
public:
	void TestEpithelialLayerUndergoingFission() throw(Exception)
	{
		...
		double end_time = 10; //Set end time (168hrs is 7days) ... This can be changed
		double sampling_timestep = 0.5/dt;//4800; //Set sampling timestep
        unsigned start_sim = 1;
        unsigned num_sims = 1; //Number of "repeated" simulations wanted to run

		/*
		 * Set the target proportion for Stem cells and Paneth cells.
		 * A proportion of 1 will most probably saturate the orgnoid with stem cells
		 * Similarly a low propotion will saturate the organoid with Paneth cells.
		 */
		double target_proportion = 0.3; //This corresponds to the 20% stem cell (80% Paneth cell) case

		/*
		 * Set the stiffness ratio for Paneth cells to stem cells.
		 * Langlands et al used 4, which means Paneth are 4 times stiffer that stem cells
		 * Almet et al propose that a stiffness of 4.5 may produce more crypts
		 */
		double stiffness_ratio_paneth = 4.5;

		/*
		 * Set the scale lenght of Paneth cell cycle.
		 * In the case of 1 it means is the same cell cycle lenght
		 * In the case of 2 it means is the double cell cycle lenght
		 */
		double cc_scale = 1.0;

		//Set the relative adhesiveness of hard cells. The values used in the paper were 1.0, 1.3 and 2.0
		/*
		 * Set the relative adhesiveness of Paneth cells.
		 * This is also a ratio relative to stem cells' adhesiveness.
		 */
		...
		/*
		 * Addition to perform several simulations in 1 run
		 */

				// Set output directory
				std::string tp = std::to_string(target_proportion);
				std::string ccs = std::to_string(cc_scale);
				std::string hcdm = std::to_string(hard_cell_drag_multiplier);
				std::string simParams = tp + "_" + ccs + "_" + hcdm;
				std::string output_directory = "CryptFission2DOrganoid_" + simParams;
				//The next string has to match the ChasteOutput Directory!!!
				std::string multiple_run_directory = "<Add your CHASTE PATH here>/testoutput/Multiple2DOrganoidRunData_" + simParams;
				std::string multi_file = "Multiple2DOrganoidRunData_" + simParams;
				...
				for(unsigned index=start_sim; index < start_sim + num_sims; index++)
				{
					...
					StochasticTargetProportionBasedCellCycleModel Stochastic_target_cell_cycle_model;
					out_stream Stochastic_target_parameter_file = output_file_handler.OpenOutputFile("Stochastic_target_results.parameters");
					Stochastic_target_cell_cycle_model.OutputCellCycleModelParameters(Stochastic_target_parameter_file);
					Stochastic_target_parameter_file->close();
					...
					for (unsigned i = 0; i < real_indices.size(); i++)
					{
						...
						std::string itrack = boost::lexical_cast<std::string>(index);
						 ...
						 //These also MUST match the Output of Chaste
						std::string oldAgeFile = "<Add your CHASTE PATH here>/testoutput/" + output_directory + "/results_from_time_0/cellages.dat";
						...
						std::string oldTypeFile = "<Add your CHASTE PATH here>/testoutput/" + output_directory + "/results_from_time_0/results.vizcelltypes";
						...
						std::string oldVolFile = "<Add your CHASTE PATH here>/testoutput/" + output_directory + "/results_from_time_0/EpithelialLayerdata.dat";
...
#endif /* TESTCRYPTFISSION2DORGANOID_HPP_ */
